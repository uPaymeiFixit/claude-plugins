...
argument-hint: '[mr-url (e.g. https://gitlab.example.com/.../merge_requests/306)]'
allowed-tools:
    - Bash(python3 *)
    - Bash(glab mr list*)
    - Bash(git *)
...
- `repoName` — the repository name (e.g. `my-service`)
...
- **repo name**: the last path segment before `/-/` (e.g. `my-service`) — or use `repoName` from the script
