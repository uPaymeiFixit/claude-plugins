...
3. On a miss, falls back to platform APIs:
    ...
    - **GitLab**: `mcp__*_gitlab__get_users` — first-initial + last name (`jappleseed`), then full first + last (`johnappleseed`), then first name only (`john`).
    - **Jira**: `mcp__claude_ai_Atlassian__lookupJiraAccountId` with `cloudId: <your-org>.atlassian.net`, searched by full name.
...
### Used by [notify-blame](../../../notify-blame/skills/notify-blame/README.md)

To resolve each blame author's git email to a Slack user ID before sending DMs.

### Used by [commit-and-mr](../../../commit-and-mr/skills/commit-and-mr/README.md)
...
### Used by [create-jira-item](../../../create-jira-item/skills/create-jira-item/README.md)
