...
| [task-pipeline:night-shift](plugins/task-pipeline/skills/night-shift/) | Work autonomously overnight — research, plan, experiment, and land straightforward features on separate branches — then deliver a brutally concise morning summary. |
| [task-pipeline:orchestrate-task](plugins/task-pipeline/skills/orchestrate-task/) | Run a medium-sized task through a model-tiered subagent pipeline — Opus plans, Fable reviews the plan, Sonnet executes, and a bundled adversarial-review workflow verifies the diff. |
| [task-pipeline:summarize-task](plugins/task-pipeline/skills/summarize-task/) | Summarize finished work for a reader who wasn't watching — outcome first, plain names, no session-invented jargon. |
